...
import copy
import random
from folder2lmdbyf import ImageFolderLMDB
...
def gen_dataset(datadir, dset_name, feature, isnumpy=False, **kwargs):
    ...
    # elif dset_name == "ilsvrc12":
    #     torch.cuda.manual_seed(42)
    #     torch.manual_seed(42)

    #     traindir = os.path.join(datadir, 'train.lmdb')
    #     testdir = os.path.join(datadir, 'val.lmdb')

    #     train_dataset = ImageFolderLMDB(traindir)
    #     test_dataset = ImageFolderLMDB(testdir)
    ...
    #     return trainset, valset, testset, num_cls
        ...
        ilsvrc_tst_transform = transforms.Compose([
          transforms.Resize(256),
          transforms.CenterCrop(224),
          transforms.ToTensor(),
          normalize])
        ...
        fullset = datasets.ImageFolder(traindir, ilsvrc_transform)
        testset = datasets.ImageFolder(testdir, ilsvrc_tst_transform)
...
#         fullset = ImageNet(root = datadir, split='train')
#         testset = ImageNet(root = datadir, split='val')

      
#         # validation dataset is (0.1 * train dataset)
#         # validation_set_fraction = 0.0   # 0.9998
#         # num_fulltrn = len(fullset)
#         # num_val = int(num_fulltrn * validation_set_fraction)
#         # num_trn = num_fulltrn - num_val
#         # trainset, valset = random_split(fullset, [num_trn, num_val])
