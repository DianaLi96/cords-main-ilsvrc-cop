...
def image_transform(image_size=224):
    ...
    # if name=='train':
    return T.Compose([T.RandomResizedCrop(image_size),
                 T.RandomHorizontalFlip(),
                 T.ToTensor(),
                 normalize])
    # For testset construction
...
def transform_reader(path):
    image_target = Image.open(path)
    transforms = image_transform()
    # try:
    # images = transforms(image_target)
    # except Exception as e:
    #     # print(e)
    rgbimg = Image.new("RGB", image_target.size)
    rgbimg.paste(image_target)
    images = transforms(rgbimg)
...
def folder2lmdb(dpath, name="train", write_frequency=5000):
    ...
    dataset = ImageFolder(directory, loader=transform_reader)
